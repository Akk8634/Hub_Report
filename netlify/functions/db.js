  ...
  try {
    ...
    if (request.method === 'POST') {
      ...
      if (!records || !records.length) {
        return new Response(JSON.stringify({ error: 'No records provided' }), { status: 400, headers });
      }
      ...
      return new Response(JSON.stringify({ success: true, saved: records.length }), { status: 200, headers });
    }
